...
test('Should call handler when message is not defined', () => {
  ...
  intl.get('not-exist-key');
  expect(warningHandler).lastCalledWith(
    'react-intl-universal key "not-exist-key" not defined in en-US',
  );
});
...
test('Default message with nested key', () => {
  intl.init({ locales, currentLocale: 'en-US' });
  expect(intl.get('NOT_EXIST_KEY.HELLO').defaultMessage('Hello World')).toBe(
    'Hello World',
  );
});
